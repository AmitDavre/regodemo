				...
				<div class="tab-pane" id="tab_contcact">
					<table class="basicTable editTable" border="0">
						<tbody>
							...
							<tr>
								<? if($data['allow_login'] == 1){$intab='readonly';}else{$intab='';} ?>
								<th><?=$lng['Personal email']?></th>
								<td><input  type="text" name="personal_email" placeholder="..." value="<?=$data['personal_email']?>" <?=$intab;?>></td>

								<td class="pl-2">
									<input type="checkbox" onclick="changesOtherChkboxP(this)" <? if($data['peComm'] == 1){ echo 'checked';}?>> <b><?=$lng['Communications']?></b>
							...
							<tr>
								...
								<td class="pl-2">
									<input type="checkbox"  onclick="changesOtherChkboxW(this)" <? if($data['weComm'] == 1){ echo 'checked';}?>> <b><?=$lng['Communications']?></b>
								...
								<td>
									<select name="username_option" onchange="getUsernameval(this.value);">
										...
										<? foreach($username_option as $k=>$v){ ?>
											<option <? if($data['username_option'] == $k){echo 'selected';}?> value="<?=$k?>"><?=$v?></option>
										<? } ?>
									</select>
								</td>
							</tr>
