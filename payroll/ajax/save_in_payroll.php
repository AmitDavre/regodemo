<?php
	...
	$sql = "SELECT * FROM ".$cid."_employees WHERE base_salary > 0 AND 
		joining_date <= '".$eom."' 
		AND (resign_date >= '".$som."' AND resign_date <= '".$eom."' OR emp_status = '1') AND emp_id IN ('".$strempids."')
	...
	if($data){
		...
		foreach($data as $k=>$v){
			...
			if($getMonthdata->num_rows > 0){
				...
				$addEmpdata = $dbc->query("INSERT INTO ".$sessionpayrollDbase." (emp_id, month, payroll_modal_id, entity, branch, division, department, team, position, emp_name_th, emp_name_en, paid_days, paid) VALUES ('".$v['emp_id']."', '".$_SESSION['rego']['cur_month']."', '".$v['payroll_modal_value']."', '".$v['entity']."', '".$v['branch']."', '".$v['division']."', '".$v['department']."', '".$v['team']."', '".$v['position']."', '".$v['th_name']."', '".$v['en_name']."', '".$calendar_days."', 'C')");
			}
		}
